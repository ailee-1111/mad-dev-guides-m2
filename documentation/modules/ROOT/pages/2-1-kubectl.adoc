...
`oc project mystuff` : 현재 활성화된 컨텍스트의 기본 네임스페이스를 mystuff로 변경합니다. +
`oc create deployment myapp --image=quay.io/rhdevelopers/quarkus-demo:v1` : quay.io/rhdevelopers/quarkus-demo:v1 이미지를 기반으로 하는 myapp이라는 이름의 `deployment` 를 생성합니다.
