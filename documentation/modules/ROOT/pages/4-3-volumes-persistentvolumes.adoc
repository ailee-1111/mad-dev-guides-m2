...
IMPORTANT: volume-%userid% 네임스페이스는 미리 생성되어 있습니다.
...
IMPORTANT: 해당 `PersistentVolume`은 관리자에 의해 미리 생성되어 있으며, `%userid%` 계정은 PV API에 대한 권한이 없습니다.
...
[source,bash]
----
watch -n1 -- "ls -al /mnt/persistent-volume11 && eval ""cat /mnt/persistent-volume11/greeting.txt"""
----
